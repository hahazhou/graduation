...
class MrpcProcessor(DataProcessor):
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
      exam = InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label)
    return exam
...
class MultiLabelTextProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        filename = 'short_train.csv'
        data_df = pd.read_csv(os.path.join(data_dir, filename), delimiter='_!_', encoding='utf-8')
        #             data_df['comment_text'] = data_df['comment_text'].apply(cleanHtml)
        return self._create_examples(data_df, "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        filename = 'short_dev.csv'
        data_df = pd.read_csv(os.path.join(data_dir, filename), delimiter='_!_', encoding='utf-8')
        #             data_df['comment_text'] = data_df['comment_text'].apply(cleanHtml)
        return self._create_examples(data_df, "dev")

    def get_test_examples(self, data_dir):
        data_df = pd.read_csv(os.path.join(data_dir, 'short_testtt.csv'), delimiter='_!_', encoding='utf-8')
...
##  predict_file = os.path.join(FLAGS.output_dir, "predicttt.tf_record")
...
##        tf.saved_model.loader.load(sess, [tag_constants.SERVING], "./path_to/1589884542")
...
##        print("hahahahahahahahahah")
  ...
  predict_fn = tf.contrib.predictor.from_saved_model("./path_to/1589884542")


##  count = 0
##    count = count + 1
##    question = input("> ")
  predict_example = InputExample(guid="1", text_a="5月29日消息，指数低开高走，三大股指均翻红。板块方面，通讯板块领涨，乳业板块经过前几日的调整后今日大幅回升。临近午盘，指数持续冲高，创指涨幅一度扩大至1.5%。盘面上，网红经济、旅游板块盘中拉升，乳业等食品加工板块出现回暖，银行、保险等权重板块回调震荡。", label=0)
